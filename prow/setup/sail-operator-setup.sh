...
# install and another is with cleanup.
# On install script is used to convert istio in-cluster operator config to sail operator and install on OpenShift with istio-cni and gateways.
# On cleanup before installed istiod, istio-cni, istio-ingressgateway and istio-engressgateway are cleaned
...
CONVERTER_ADDRESS="https://raw.githubusercontent.com/ctartici/sail-operator/main/tools/configuration-converter.sh"
